...
int isInArray(const char *vars, const char c){
	if(c == vars[0])
		return 0;
...
int parseFunct(Function *f, int *idxE){
	...
	while(i < f->len){
		...
		switch(f->str[i]){
			...
			case '*':
			case '/':
			case '^': /* HERE I consider ^ as an operator with function's priority */
				if(top >= 0){
					stItm = stack[top];
					...
					while((stItm->function == F_OPT ) && (stItm->priority) >= getPriority(f->str[i])){
						...
						/*printf("(i:%d)A Popped %c from Stack (top: %d )\n", i, stItm->chr, top);*/
						itm = (TNode*)malloc(sizeof(TNode));
				...
				*/
				...
				/*printf("(i:%d) Pushed %c (priority: %d) to Stack (top:%d)\n", i, f->str[i], stItm->priority, top);*/
				i++;
				break;
			...
			case '(':/*If it an open parentheses then put it to stack*/
				...
				push2Stack(&stack, &top, &allocLen, OPN, F_PARENT, NO_PRIORITY);
				/*printf(" Pushed '(' to Stack at top = %d \n", top);*/
				i++;
				break;

			case ')':
				stItm = popFromStack(stack, &top);
				/*printf("(i:%d) Popped %c from Stack (top after: %d )\n", i, stItm->chr, top);*/
				...
				/*	while(stItm!=NULL && stItm->function != F_PARENT){*/
					addFunction2Tree(f, stItm);
					/*printf("(i:%d) Add %c from Stack (top: %d )\n", i, stItm->chr, top);*/
					free(stItm);
					stItm = popFromStack(stack, &top);
					/*printf("(i:%d) Popped %c from Stack (top after: %d )\n", i, stItm->chr, top);*/
				}
				
				/*printf("(i:%d) SHOULD BE ( and got %c (top : %d ) %c\n", i, stItm->chr, top, stack[top]->chr);*/
			...
			case 's':
				...
				if(f->str[i+1]=='q' && f->str[i+2]=='r' && f->str[i+3]=='t' &&
					...
					/*printf("(i:%d) Pushed sqrt( to Stack (top: %d)\n", i, top);*/
					i += 5;
					break;
				}
				/* Syntax error */
			...
			case 'c':
				...
				if(f->str[i+1]=='o' && f->str[i+2]=='s' && f->str[i+3]=='('){
					...
					/*printf("(i:%d) Pushed cos( to stack (top:%d)\n", i, top);*/
					i += 4;
					break;
				}
			...
			default: //variables
				if(isVariable(f, f->str[i]) >= 0){
					addValue2Prefix(f, f->str[i], F_VAR, NO_PRIORITY, 0);
					/*printf("(i:%d) Adding variable %c to Prefix \n", i, f->str[i]);*/
					i++;
	...
	if( (dp->t)->function == F_OPT ){
		if(((dp->t)->left!=NULL) && ((dp->t)->right!=NULL)){
			...
			if( (dp->t)->chr == PLUS){
				...
				/*printf(" dp->t address: %p, Right address %p , value %lf \n", (dp->t), (dp->t)->right, (dp->t->right)->value);*/
				...
				if( ((dp->t)->left)->function == F_COE && ((dp->t)->left)->value == 0 ){
					...
					if((dp->t)->right != NULL){
						...
						(dp->t)->left = p->left;
						(dp->t)->right = p->right;
						/*printf("090 dp->t address: %p, Right address %p \n", (dp->t), (dp->t)->right);*/
						/* Now release p */
						free(p);
						p = NULL;
						return &(dp->error);
				...
				/*=============================*/
				/*printf(" dp->t address: %p, Right address %p \n", (dp->t), (dp->t)->right);*/
				/*=============================*/
			...
			/* ^ is a function but act as an operator */
			if((dp->t)->chr == POW){
	...
	if( ((dp->t)->function == F_FUNCT)){
		switch( (dp->t)->chr ){
			...
			case SQRT:
				...
				if( ((dp->t)->right != NULL) && (((dp->t)->right)->function==F_COE)){
...
void nodeToString(TNode *t, char *str, int len, int *newlen){
	...
	if( (*newlen)<=0 ){
		if(dval == 0.0)
			l = sprintf(tmp, "%d;%d;%ld", t->function, t->chr,(long)(t->value));
		else
