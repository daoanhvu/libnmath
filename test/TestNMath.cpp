...
void testFunction0() {
	...
	int error = f.parse("f(mX,mY)=sin(mX)*mY", 15, &lexer, &parser);
...
void testFunction1() {
	...
	if (dataFile.is_open()) {
		while (getline(dataFile, line)) {
			...
			if (error != NMATH_NO_ERROR) {
				std::cout << "NFunction " << lineCount << " Parsing ERROR = " << error << "\n";
			}
			else {
				//std::cout << f;
				...
				if (data != NULL) {
					...
					std::cout << "NFunction " << lineCount << " Get Space Failed \n";
				}
				std::cout << "\nNFunction Parsing OK\n";
			}
			std::cout << "\n******************************************************\n";
			lineCount++;
		}
