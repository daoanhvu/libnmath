...
private:
  ...
  std::vector<ImageData<T>*> getSpaceFor2UnknownVariables(const T *inputInterval, T epsilon,
    ...
    for(int i=0; i<postfix.size(); i++) {
      ...
      if(criteria[i] == nullptr) {
        ...
        outCriteria = (CompositeCriteria<T>*)criteria[i]->getInterval(inputInterval, this->variableNames);
   ...
   * @param prefixIndex
   * @param inputInterval
   * @param epsilon the space between two vertices in one dimension
  ...
  ImageData<T>* getSpaceFor2WithANDComposite(int prefixIndex, const T *inputInterval,
    ...
    param.t = postfix[prefixIndex];
    param.varCount = (unsigned int)variables.size();
...
public:
  ...
  int toString(char *str, int buflen) {
    int start = 0;
    nmath::toString(postfix[0], str, &start, buflen);
    return start;
  }
  
  /**
   functionNotation should be in pattern NAME LPAREN NAME (COMA NAME)* PRARENT;
   */
  bool startWithFunctionNotation(std::vector<Token*> tokens, int start, int *nextIndex, int *errorCd) {
    size_t tokenCount = tokens.size();
    *errorCd = ERROR_NOT_A_FUNCTION;
    if( (tokenCount < 4) || (start < 0) || (start >= tokenCount-3) ) {
      *errorCd = ERROR_NOT_A_FUNCTION;
      return false;
    }
    ...
    if(tokens[index]->type == NAME ) {
      *errorCd = ERROR_PARENTHESE_MISSING;
      if(tokens[index+1]->type == LPAREN) {
        *errorCd = ERROR_MISSING_VARIABLE;
        if(tokens[index+2]->type == NAME) {
          index += 3;
          while( (index+1<tokenCount) && (tokens[index]->type == COMMA) && (tokens[index+1]->type == NAME ) ) {
            index += 2;
          }
          *errorCd = ERROR_PARENTHESE_MISSING;
          if( (index < tokenCount) && (tokens[index]->type == RPAREN)){
            *errorCd = NMATH_NO_ERROR;
            *nextIndex = index + 1;
            return true;
          }
        }
  ...
  int parse(const char *str, unsigned int len, NLabLexer *mLexer, NLabParser<T> *mParser) {
    ...
    mParser->parseFunctionExpression(mTokens, postfix,
  ...
  int parse(std::vector<Token*> tokens, NLabParser<T> *mParser) {
    ...
    mParser->parseFunctionExpression(tokens, postfix, domain, variables, &errorCode, &errorColumn);
    if (errorCode == NMATH_NO_ERROR) {
      for(i=0; i<variables.size(); i++) {
        //this->variableNames[i] = variables[i]->text;
        this->variableNames.push_back(variables[i]->text);
      }
      for (i = 0; i < domain.size(); i++) {
        c = nullptr;
        if (domain[i] != nullptr) {
   ...
   * @return base on the expression of the function, we pass values to variables and calculate the function's value
   */
  ...
  std::vector<ImageData<T>*> getSpace(const T *inputInterval, T epsilon, bool needNormalVector, bool needNormalizeNormalVector) {
    ...
    switch (valLen) {
      case 1L:
        ...
        outCriteria = criteria[0]->getInterval(inputInterval, this->variableNames);
  ...
  /**
   fidx NFunction expression index (the index of postfix.list)
   vidx variable index
   ...
   * TODO:
   * */
  NMAST<T>* getPrefixList() const { return postfix[0]; }
  NMAST<T>* getPrefix(int idx) const { return postfix[idx]; }
